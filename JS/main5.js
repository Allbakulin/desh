...
var switchClass = 0;
		...
		var companyName = ["D","E","S","I","G","N","E","M","B","R","A","C","E","D"]
		var companyBuildUp="";
		var showLetter="";
		var perc2;
		function update() {
										...
										(function(p) {
											setTimeout(function(){
												imagesLoadedCount++
													...
													//var id = document.getElementById("image"+p);
													showLetter = companyName[parseInt(imagesLoadedCount/Model.items.length*13)];
												
													//companyBuildUp+=showLetter;
													
													percentage = parseInt(imagesLoadedCount/Model.items.length*100);
													
													//$siteLoader.css("width", percentage+"%");
													
													$siteLoader.css({"transform": "translate3d(" + percentage + "%, 0px, 0px)"});
													
													perc2 = Math.round($siteLoader.width()/$(window).width()*100);
													
													if(percentage<10) {
														$loaderText.text(showLetter+"-"+"0"+Math.round(percentage));
													} else {
														$loaderText.text(showLetter+"-"+Math.round(percentage));
													}
													
													//$companyNameType.text(companyBuildUp);
													$loaderTextAnmount.text(imagesLoadedCount+"/"+Model.items.length+" images");
													if (imagesLoadedCount==Model.items.length && percentage==100) {
														$loaderText.text("D-100");
													
														
													setTimeout(delayCollapseLoader, 800);
									}
												}, 200 * p);
										})(i);
									}
...
function delayCollapseLoader(){
	if ($(window).width()<800) {
		...
		//$siteLoader.css("margin-left", "100%");
